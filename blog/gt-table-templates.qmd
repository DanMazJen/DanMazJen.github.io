...
The content of the table is often explicitly stated in the statistical analysis plan (SAP). In planed studies with FDA/EDA approval, such tables are submitted in advance of conducting the study in order to get the approval. Even in the case of smaller experiments, researchers are well aware of the relevant background information which goes into the Table 1.
...
table1_template |>
  ...
  gt::cols_label(
    ...
    ra_ild = "RA-ILD",
    ipf = "IPF",
    other_ild = "Other type"
  ) |>

  gt::tab_spanner(
    label = "ILD subtype",
    columns = c(ra_ild, ipf, other_ild)
  ) |>
  ...
  gt::tab_header(
    title = gt::md(
      "**Table 1. Descriptive characteristics of patients with lung cancer, Denmark, 2015–2024**"
    )
  ) |>
  ...
  gt::tab_source_note(
    source_note = gt::md(
      ...
      ILD: Interstitial Lung Disease; IQR: Interquartile Range; SCLC: Small Cell Lung Cancer; NSCLC: Non-Small Cell Lung Cancer; ECOG: Eastern Cooperative Oncology Group Performance Status. ^ Proportions differ between the Danish Cancer Registry (DCR) and the Danish Lung Cancer Registry (DLCR) due to differences in inclusion criteria for lung cancer diagnoses.*"
    )
  )
```
...
```{.r}
table1_template$no_ild[table1_template$label == "Total, N"] <- "12,431"
```
...
table1_template <- fill_cell(table1_template, "Total, N", "no_ild", "12,431")
```
